...
- **Styles:** `app/styles/globals.css` imports Tailwind v4, declares shadcn's `@theme`, then ports the marketing CSS under `@layer base` (preserves `:root` tokens like `--red`/`--ink` that drive every marketing component). **`@utility container` at the bottom of the file is not redundant** — Tailwind v4 ships a built-in `.container` utility in `@layer utilities` that overrides `@layer base` styles, silently clamping the container to breakpoint widths (e.g. `1024px` at `lg`). The `@utility container` block replaces that built-in with our `1240px` values. Do not remove it.
